...
BOOL CBasicMeshObject::CreateMesh()
{
	...
	HRESULT hCreateCommittedResource = pD3DDevice->CreateCommittedResource(
	...
	if (FAILED(hCreateCommittedResource))
	{
		__debugbreak();
	}
	...
	HRESULT hMap = m_pVertexBuffer->Map(0, &readRange, reinterpret_cast<void**>(&pVertexDataBegin));
	if (FAILED(hMap))
	{
		__debugbreak();
	}
